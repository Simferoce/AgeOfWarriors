...
namespace Game
{
    public class AttackFactory : MonoBehaviour
    {
        ...
        public Attack Generate(float damage, float armorPenetration, float leach, bool ranged, bool overtime, bool reflectable, Attackable attackable)
        {
            ...
            {
                float damageDealtReduction = modifiers.Max(x => x.StatisticRegistry.TryGetStatistic<float>(StatisticDefinition.PercentageDamageDealt, out Statistic<float> statistic) ? statistic.GetValue() : 0f);
            ...
            if (attackable.Entity.GetCachedComponent<ModifierHandler>().GetModifiers().Any(x => x is DamageDealtReductionModifierDefinition.Modifier))
                damage += Entity.GetCachedComponent<ModifierHandler>().GetModifiers().Sum(x => x.StatisticRegistry.TryGetStatistic<float>(StatisticDefinition.PercentageDamageDealtAgainstWeak, out Statistic<float> statistic) ? statistic.GetValue() : 0f);
